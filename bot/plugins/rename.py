...
async def rename_handler(c: Client, query: CallbackQuery):
    ...
    if not query.from_user:
        return await query.reply_text("I don't know about you, sir :(")

    if query.from_user.id not in Config.PRO_USERS:
        is_in_gap, sleep_time = await check_time_gap(query.from_user.id)
        if is_in_gap:
            return await query.reply_text(f"Sorry Sir, No Flooding Allowed!\n\nSend After `{str(sleep_time)}s` !!", quote=True)

    await add_user_to_database(c, query)
    
    # Proceed
    editable = await query.reply_text("Now send me a new file name!", quote=True)
    user_input_msg: Message = await c.listen(query.chat.id)
    ...
    _raw_file_name = get_media_file_name(query.reply_to_message)

    if not _raw_file_name:
        _file_ext = mimetypes.guess_extension(get_file_attr(query.reply_to_message).mime_type)
    ...
    is_big = get_media_file_size(query.reply_to_message) > (10 * 1024 * 1024)

    if not is_big:
        _default_thumb_ = await db.get_thumbnail(query.from_user.id)
        if not _default_thumb_:
            _m_attr = get_file_attr(query.reply_to_message)
            _default_thumb_ = _m_attr.thumbs[0].file_id if (_m_attr and _m_attr.thumbs) else None

        await handle_not_big(c, query, get_media_file_id(query.reply_to_message), file_name,
                             editable, get_file_type(query.reply_to_message), _default_thumb_)
        return

    file_type = get_file_type(query.reply_to_message)
    _c_file_id = FileId.decode(get_media_file_id(query.reply_to_message))

    try:
        c_time = time.time()
        file_id = await c.custom_upload(
            file_id=_c_file_id,
            file_size=get_media_file_size(query.reply_to_message),
            file_name=file_name,
        ...
        await handle_big_rename(c, query, file_id, file_name, editable, file_type)
