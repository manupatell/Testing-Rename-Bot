...
@Client.on_message(filters.command("editmetadata") & filters.private & ~filters.edited)
async def edit_Metadata(c: Client, m: Message):
    await add_user_to_database(c, m)
    if not m.reply_to_message or len(m.command) == 1:
        await m.reply_text("**Reply to a video with /editmetadata**", True)
        return
    ...
    dl_loc = f"{Config.DOWNLOAD_DIR}/{user_id}/{m.message_id}/"
    root_dl_loc = dl_loc
    ...
    the_media = await c.download_media(
        ...
        progress_args=("Downloading ...", editable, c_time)
    )
    ...
    try:
        ...
        # Clean Up
        await editable.edit(f"Failed to process video! Error: {e}")
        await rm_dir(root_dl_loc)
        return
    ...
    _default_thumb_ = await db.get_thumbnail(user_id)
