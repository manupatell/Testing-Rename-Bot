...
async def video_info_handler(c: Client, m: Message):
    ...
    video_title = (await db.get_titles(m.from_user.id)) or "StarMovies.hop.sh"
    audio_title = (await db.get_titles(m.from_user.id)) or "StarMovies.hop.sh"
    subtitle_title = (await db.get_titles(m.from_user.id)) or "StarMovies.hop.sh"
